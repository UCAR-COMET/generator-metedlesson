...
// * Satellite Applications for Marine Fog (10172)
// ********************************************************************* //
...
$(function () {
	...
	$(".try-again").on("click", function () {
		...
		$(this).siblings(".submit-button").prop("disabled", false);
		$(this).siblings(".submit-button").switchClass("btn-default", "btn-primary");
		$(this).hide();
	});
	...
	$('.radio .submit-button').click(function () {
		...
		if (my_boolean) {
			...
			if (!$(selected_input).parent().hasClass("correct")) {
				$(selected_input).parent().addClass('incorrect-answer')
			}
	...
	$('.checkboxes .submit-button').click(function () {
		...
		for (i = 1; i < this.form.length - 1; i++) {
			if (this.form[i].checked == true) {
				selected_inputs.push(this.form[i])
				my_boolean = true;
			}
		}//end for loop
		...
		if (my_boolean) {
			attempts++;
			$(this).siblings(".response-group-item").each(function () {
				if ($(this).hasClass("correct") && $(this).children("input").is(':checked')) {

				} else if ($(this).hasClass("correct") && !$(this).children("input").is(':checked')) {
					isCorrect = false
				} else if (!$(this).hasClass("correct") && $(this).children("input").is(':checked')) {
					isCorrect = false
				}
			...
			if (attempts < 2 && !isCorrect) {
				$(this).parent().parent().siblings('.message').hide();
				$(this).siblings(".try-again").show();
				$(this).attr('disabled', 'disabled').addClass('btn-default').removeClass('btn-primary');
			} else {
				$(this).attr('disabled', 'disabled').addClass('btn-default').removeClass('btn-primary');
				$(this).parent().parent().parent().children('.answer')
				...
				$(this).siblings('.correct')
					.addClass('correct-answer');
				for (j = 0; j < selected_inputs.length; j++) {
					if (!$(selected_inputs[j]).parent().hasClass("correct")) {
						$(selected_inputs[j]).parent().addClass('incorrect-answer')
					}
				}
				$(this).closest('.question-wrap').next('.show-after').show();
				$(this).closest('.interaction-question').next('.show-after').show();
	...
	$('.inlinedropdowns .submit-button').click(function () {
		...
		if (my_boolean) {
			...
			//check for correct or incorrect responses
			$(this).parents('.inlinedropdowns').find('.response-group-item').each(function () {
				if ($(this).hasClass("answer")) {
					...
					var correctAnswer = $(this).next('.answer').text()
						.replace(/\s+/g, '')
						.replace(/\|/g, '')
						.replace('Thecorrectanswersare:', '')
						.replace('Thecorrectansweris:', '');
					$(this).find("select").each(function () {
						itemSelected += $(this).children('option:selected').text().replace(/\s+/g, '');
					})
					//Mark response as correct or incorrect
					if (itemSelected === correctAnswer) {
