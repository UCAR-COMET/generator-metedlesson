...
$(function () {
	$('.drawing-question').each(function() {
	   ...
	   $(this).append('<div class="drawing-container center-block" title="' + drw3Path + drw3Image + '" ></div>');
			$(this).children('.drawing-container').append('<div class="canvas-wrap row"></div>');
...
// DRAW TOOL 2.O - Revised 06/29/15
// Log: Added responsive functionality - Bryan
...
//		saveResizeAndRedisplay();
		...
		function brushIdentifier() {
			if($(this).parents(".drawing-container").find("input[name='draw-brush']:checked").val() == "Eraser") {
				$(this).parents(".drawing-container").find(".drawtool-drawsize").append("<span class='multiplier-count'>x4</span>");
			}else {
				$(this).parents(".drawing-container").find(".drawtool-drawsize").children('.multiplier-count').remove();
			}
		}
		...
		$(".drawsize").change(function(a){
			...
			var d=$(".drawing-canvas")[i];
			var e=d.getContext("2d");
			var brushType = $(this).parents(".drawing-container").find("input[name='draw-brush']:checked").val();
			// eraser size multiplier
			if (brushType == "Eraser") {
				e.lineWidth=this.value *4;
			} else {
		...
		$(".drawtool-palette").change(function(a){
			...
			// eraser size multiplier
			if ($(this).parents(".drawing-container").find("input[name='draw-brush']:checked").val() == "Eraser"){
				e.lineWidth=$(this).parents(".drawing-container").find(".drawtool-drawsize").children("select:first").val() *4;
			} else {
				e.lineWidth=$(this).parents(".drawing-container").find(".drawtool-drawsize").children("select:first").val();
			}
			brushIdentifier();
